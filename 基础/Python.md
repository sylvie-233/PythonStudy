...
std:
    ...
    ftplib:
    functools: # 函数工具库
        @cache(): # 等价于lru_cache(maxsize=None)
        @lru_cache(): # 将函数的参数 做哈希处理，作为 key，将函数返回值 存储到一个 dict-like 的结构中，如果缓存满了（达到了 maxsize），就淘汰最近最少使用的项
            maxsize:
        @singledispatch(): # 支持方法重载
            register(): # 注册重载方法 参数类型
        @total_ordering(): # 自动生成缺失的比较运算符
...
本质上就是一个 .dll 文件，只不过扩展名改成 .pyd，让 Python 能识别并导入（通常是用 C/C++、Cython 或其他语言 写的扩展模块，编译后生成 .pyd，然后可以像普通 Python 模块一样 import）
- C 扩展编译生成
- Cython 生成：用 Cython 写 .pyx 文件，编译后会输出 .pyd
- 打包分发
...
Cython 是一种让你用接近 Python 的语法写 C 扩展的语言
完全兼容python语法
...
cython:
    cdef: # 变量、函数声明（C 内部函数）
        ...
        list:
        class:
        extern from: # 引入c文件（头文件）
            namespace:
    cimport:
    cpdef: # 变量、函数声明（即可在Python中、也可在C中）
```
